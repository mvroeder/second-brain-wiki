...
Copy `templates/wiki-vault-CLAUDE.md` to `~/Obsidian/<WIKI_VAULT>/CLAUDE.md`, replacing the
...
`templates/obsidian-root-CLAUDE.md` to `~/Obsidian/CLAUDE.md`.

## 4. Local state for the triage jobs
Create `~/Cowork/productivity/` and copy `templates/routing-rules.md` there (token-substituted).
The JSONL ledgers (`ingest-ledger`, `newsletter-ledger`, `reading-list-ledger`) are created on
first run. `reading-list-triage` is **optional** - it reads a `TASKS.md` reminders-sync file from the
same folder. If you do not run a Reminders -> TASKS.md sync, skip this job; `reading-list-ledger`
and `TASKS.md` are then not needed.
